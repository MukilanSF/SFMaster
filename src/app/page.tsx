...
export default function Home() {
  ...
  const [selected, setSelected] = useState<{ category: string; level: string } | null>(null);
  ...
  const handleRun = async (code: string) => {
    ...
    try {
      ...
      setRunResult(result);
      // Save submission to Supabase with pass/fail status
                ...
                <div className="mb-4">
                  ...
                  <ul className="list-disc ml-6 text-gray-600">
                    {Array.isArray(selectedQuestion.hints)
                      ? selectedQuestion.hints.map((hint: string, i: number) => (
                          <li key={i}>{hint}</li>
                        ))
